...
with open(PICKLEPATH+'/doc_vox1.pickle', 'rb') as f:
    # Pickle the 'data' dictionary using the highest protocol available.
...
test_set = con_test_set[:20] + lib_test_set[:20]
...
no_con_docs = len ([c for c in lib_best[:100] if c[0]< train_cutoff] )

# account for the fact that there are more con doc than lib docs
# ratio = len(lib_test_set)/len(con_test_set)
print ("Number of top 100 matching con docs :", no_con_docs)
